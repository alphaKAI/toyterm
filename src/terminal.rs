            ...
            loop {
                ...
                if let Some(flags) = ctl_revents {
                    if flags.contains(PollFlags::POLLIN) {
                        ...
                        || flags.contains(PollFlags::POLLHUP)
                    {
                        let _ = kill(self.pid, Signal::SIGHUP);
                        break;
                    }
                    ...
                    ED(ps) => match ps {
                        0 => {
                            ...
                            state.images.retain(|img| {
                                let v_cells =
                                    ((img.height as u32 + cell_hpx - 1) / cell_hpx) as isize;
...
mod terminal_impl {
    use portable_pty::{
        native_pty_system, ChildKiller, CommandBuilder, ExitStatus, PtySize,
    };

    use std::io::{Read, Write};

    enum MasterWriteRequest {
        Resize { rows: u16, cols: u16 },
        Write(Vec<u8>),
        Terminate
    }
    enum MasterReadRequest {
        Read,
        Terminate
    }
    enum MasterReadResponse {
        ReadResult(Vec<u8>),
        ChildExit(ExitStatus),
    }

    struct Pty {
        pub master_write_req: crossbeam_channel::Sender<MasterWriteRequest>,
        pub master_read_req: crossbeam_channel::Sender<MasterReadRequest>,
        pub master_read_res: crossbeam_channel::Receiver<MasterReadResponse>,
    }
    ...
    impl Pty {
        pub fn clone(&self) -> Self {
            Self {
                master_write_req: self.master_write_req.clone(),
                master_read_req: self.master_read_req.clone(),
                master_read_res: self.master_read_res.clone(),
            }
        }
    }
    ...
    pub struct Terminal {
        pty: Pty,
        control_req: crossbeam_channel::Sender<Command>,
        control_res: crossbeam_channel::Receiver<i32>,
        pub state: Arc<Mutex<State>>,
    }

    impl Terminal {
        pub fn new(size: TerminalSize, cell_size: CellSize, cwd: &std::path::Path) -> Self {
            let (pty, child_killer) = init_pty(cwd, &size).unwrap();

            let (control_req_tx, control_req_rx) = crossbeam_channel::unbounded();
            let (control_res_tx, control_res_rx) = crossbeam_channel::unbounded();
        ...
        pub fn pty_write(&mut self, data: &[u8]) {
            log::trace!("pty_write: {:x?}", data);
            //self.pty.fd.write_all(data).unwrap();
            self.pty
                .master_write_req
                .send(MasterWriteRequest::Write(data.to_vec()))
                .expect("write fail");
            log::trace!(" -> [ok] pty_write: {:x?}", data);
        }

        pub fn request_resize(&mut self, buff_sz: TerminalSize, cell_sz: CellSize) {
            log::debug!("request_resize: {}x{} (cell)", buff_sz.rows, buff_sz.cols);
            self.control_req
                .send(Command::Resize { buff_sz, cell_sz })
                .unwrap();
            self.control_res.recv().unwrap();
        }

        pub fn send_sigterm(&mut self) {
            self.control_req.send(Command::SendSigterm).unwrap();
            self.control_res.recv().unwrap();
        }
    ...
    struct Engine {
        ...
        pty: Pty,
        control_req: crossbeam_channel::Receiver<Command>,
        control_res: crossbeam_channel::Sender<i32>,
        cell_sz: CellSize,
        state: Arc<Mutex<State>>,
        parser: control_function::Parser,
        tabstops: Vec<usize>,
    ...
    impl Engine {
        fn set_term_window_size(pty_master: &Pty, size: TerminalSize) -> Result<()> {
            pty_master
                .master_write_req
                .send(MasterWriteRequest::Resize {
        ...
        fn start(mut self) {
            ...
            loop {
                ...
                self.pty
                    .master_read_req
                    .send(MasterReadRequest::Read)
                    .expect("failed to request read");

                crossbeam_channel::select! {
                    recv(self.control_req) -> res => match res {
                        Ok(res) => match res {
                            Command::Resize { buff_sz, cell_sz } => {
                                self.resize(buff_sz, cell_sz);
                                self.control_res.send(0).unwrap();
                            }
                            Command::SendSigterm => {
                                let _ = self.child_killer.kill();
                                // self.child.wait().expect("wait failed");
                                if let Ok(MasterReadResponse::ChildExit(exit_status)) = self.pty.master_read_res.recv() {
                        ...
                        _ => {
                            self.child_killer.kill().unwrap();
                        }
                    },
                    recv(self.pty.master_read_res) -> res => match res {
                        Ok(res) => {
                            let mut buf = match res {
                                MasterReadResponse::ReadResult(buf) => {
                                    if buf.len() == 0 {
                                        break;
                                    } else {
                                        buf
                                ...
                                MasterReadResponse::ChildExit(exit_status) => {
                                    log::trace!("child exit : {exit_status:?}");
                                    opt_exit_status = Some(exit_status);
                                    break;
                                }
                            };
                        ...
                        _ => {
                            self.child_killer.kill().unwrap();
                            log::debug!("I'm not sure killing child is valid behavior");
                        }
                    }
                }
            }

            //let status = self.child.wait().expect("failed to wait").exit_code() as i32;
            let status = if let Some(exit_status) = opt_exit_status {
                exit_status.exit_code() as i32
            } else {
                if let Ok(MasterReadResponse::ChildExit(exit_status)) =
                    self.pty.master_read_res.recv()
                {
                    exit_status.exit_code() as i32
                } else {
                    -1
                ...
                match func {
                    ...
                    DSR(ps) => match ps {
                        5 => {
                            ...
                            self.pty
                                .master_write_req
                                .send(MasterWriteRequest::Write(b"\x1b[0\x6E".to_vec()))
                                .unwrap();
                        }
                        6 => {
                            let (row, col) = state.cursor.pos();
                            ...
                            self.pty
                                .master_write_req
                                .send(MasterWriteRequest::Write(
                                    format_args!("\x1b[{};{}\x52", row + 1, col + 1)
                                        .to_string()
                                        .into_bytes()
                                        .to_vec(),
                                ))
    ...
    fn init_pty(
        _cwd: &std::path::Path,
        size: &TerminalSize,
    ) -> Result<(Pty, Box<dyn ChildKiller + Send + Sync>)> {
        ...
        let mut child = {
            // spawn shel in slave,  Setup process states and execute shell
            let mut shell = CommandBuilder::from_argv(
            ...
            for (k, v) in vars.iter() {
                shell.env(OsString::from(k), OsString::from(v));
            }
        ...
        let mut master_reader = pair
            .master
            .try_clone_reader()
            .expect("failed to take reader");
        let mut master_writer = pair.master.take_writer().expect("failed to take writer");

        let (master_write_tx, master_write_rx) = crossbeam_channel::unbounded();
        {
            std::thread::spawn(move || loop {
                match master_write_rx.recv() {
                    Ok(req) => match req {
                        MasterWriteRequest::Resize { rows, cols } => pair
                            .master
                            .resize(PtySize {
                                rows,
                                cols,
                            ...
                            .expect("failed to resize"),
                        MasterWriteRequest::Write(buf) => {
                            master_writer.write_all(&buf).expect("failed to write");
                        },
                        MasterWriteRequest::Terminate => {
                            log::trace!("recv master write request thread terminate");
                            break;
                        }
                    },
                    Err(err) => {
                        log::trace!("recv master write request failed {err:?}");
                    }
                }
            });
        }

        let (master_read_req_tx, master_read_req_rx) = crossbeam_channel::unbounded();
        let (master_read_res_tx, master_read_res_rx) = crossbeam_channel::unbounded();
        {
            let master_read_res_tx = master_read_res_tx.clone();
            std::thread::spawn(move || loop {
                let mut buf: [u8; 1024] = [0; 1024];
                match master_read_req_rx.recv().expect("failed to recv") {
                    MasterReadRequest::Read => {
                        let n = master_reader
                            .read(&mut buf)
                            .expect("failed to read from master_read");
                        master_read_res_tx
                            .send(MasterReadResponse::ReadResult(buf[..n].to_vec()))
                            .expect("failed to send read result");
                    },
                    MasterReadRequest::Terminate => {
                        log::trace!("recv master read request thread terminate");
                        break;
                    }
                }
            });
        ...
        {
            let master_write_req_tx = master_write_tx.clone();
            let master_read_res_tx = master_read_res_tx.clone();
            let master_read_req_tx = master_read_req_tx.clone();
            std::thread::spawn(move || match child.wait() {
                Ok(exit_status) => {
                    master_write_req_tx
                        .send(MasterWriteRequest::Terminate)
                        .unwrap();
                    master_read_req_tx
                        .send(MasterReadRequest::Terminate)
                        .unwrap();
                    master_read_res_tx
                        .send(MasterReadResponse::ChildExit(exit_status))
                        .unwrap();
                }
                Err(err) => {
                    panic!("failed to wait child : {err:?}");
                }
            });
        }
        ...
        Ok((
            Pty {
                master_write_req: master_write_tx,
                master_read_req: master_read_req_tx,
                master_read_res: master_read_res_rx,
            },
            child_killer,
        ))
    }
